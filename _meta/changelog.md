...
- **AYD-006** — Partner app (auth + rotating QR): draft design closing the `Partner` side of the redemption flow. Introduces the `PartnerOperator` domain entity (email-bound to a single `Partner`, `firebase_uid` bound on first login), an internal admin endpoint to pre-register operators, a scoped `GET /partner/me` context endpoint, and `GET /partner/me/totp-secret` that delivers the AYD-004 per-`Partner` TOTP secret to the authenticated app for local, offline QR computation (ADR-004). Pins the concrete TOTP parameters (SHA1/6 digits/30s/±1 window) and the QR payload encoding shared by the Partner app (produces), the Subscriber app (consumes — AYD-007) and the api (recomputes). Covers RF-15/RF-17; RF-16 (in-app metrics) deferred to AYD-008. Draft — pending review.
