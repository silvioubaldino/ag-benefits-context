...
- **AYD-009 + GLO** — Business metrics design: the `PartnerOperator` sees the own `Partner`'s `Redemption`s and `Savings` per period in the app (RF-16), and the business pulls aggregates by `Partner`/`Region`/`Benefit`/day as JSON or CSV, plus a pilot engagement overview (RF-14). Read-only projections recomputed from the immutable `Redemption` table — never from telemetry — with no new entity or column, no PII and no anti-fraud leakage. Adds `RedemptionMetrics` to the glossary to separate business numbers from observability metrics, and defers `MRR`/churn to a product decision. Draft — generates SPEC-009@api and SPEC-009@mobile.
