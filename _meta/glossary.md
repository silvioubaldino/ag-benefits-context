...
| **`RedemptionMetrics`** | Agregação de `Redemption`s e `Savings` num recorte (período × `Partner`/`Region`/`Benefit`). É **projeção de leitura** derivada da tabela de `Redemption` — não é entidade persistida nem fonte da verdade. **Métrica de negócio**, distinta da *telemetria* técnica (logs/contadores/traces — ADR-005), que é amostrável e não serve de número de negócio. (PT: "métricas de negócio") | "metrics" genérico (colide com telemetria), "analytics", "report", "dashboard", "stats", "relatório" |
